...
当前发布的 macOS 安装包为 **adhoc 签名、未做 Apple 公证**，因此首次打开会被 Gatekeeper 拦一次。
按下面三步做，**只需做一次**：

1. 打开 `.dmg`，把 `OrgCompass` **拖进「应用程序」文件夹**。
   ⚠️ 不要在磁盘映像窗口里直接双击运行 —— 直接在映像内运行会报
   「"OrgCompass"已损坏，无法打开。你应该推出磁盘映像。」
2. 打开「终端」，执行一次（复制粘贴后回车，需要输入开机密码）：

   ```bash
   xattr -dr com.apple.quarantine /Applications/OrgCompass.app
   ```

3. 之后从「应用程序」正常打开即可。

> **为什么需要这行命令**：从浏览器 / 微信等下载的文件会被 macOS 打上 quarantine（隔离）标记；
> 本应用是 adhoc 签名、无 Developer ID、未公证，Gatekeeper 会直接把它判定为「已损坏」。
> 该命令只移除这一个标记，不修改应用内容。
>
> 如果提示的是「**无法验证开发者**」而不是「已损坏」，可改用「访达」里**右键 → 打开**，
> 或到「系统设置 → 隐私与安全性」点「仍要打开」。
>
> **磁盘映像推不出**（提示「未能推出含有"OrgCompass"的磁盘映像」）：说明还有进程占着它。
> 先关掉报错窗口和「访达」里的映像窗口，再点推出；仍不行则执行
> `diskutil unmount force /Volumes/OrgCompass`。
>
> 彻底解决办法是购买 Apple Developer ID 并做公证（每年 99 美元）—— 那样用户双击即可打开、
> 无需任何命令。这是当前版本的已知限制，不影响应用功能与本地数据。

#### Windows 安装步骤

1. 下载 `OrgCompass_<版本>_x64-setup.exe`，双击安装。默认装到当前用户，**不需要管理员权限**。
2. 若出现「Windows 已保护你的电脑」：点「**更多信息**」→「**仍要运行**」。
3. 安装过程**需要联网**：Tauri 应用依赖 WebView2 运行时，目标机缺失时安装器会自动下载。

> `.msi` 是给 IT 统一部署用的（需管理员权限，支持 `msiexec /qn` 静默安装与组策略下发）；
> 自己用选 `.exe` 即可。Windows 安装包同样未做代码签名，可能出现 SmartScreen 提示。
>
> 仅支持 64 位 Windows（`x64` 指 CPU 架构，Intel / AMD 均可）；
> ARM 版 Windows 会以 x64 仿真方式运行，可用但非原生。
